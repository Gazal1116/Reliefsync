...
function RequesterDashboard() {
  ...
  const fetchUserRequests = async () => {
    if (!token || !userId || !isValidObjectId(userId)) {
      setLoading(false);
      setError("Session issue detected. Please log in again.");
      return;
    }
    ...
    try {
      ...
      setError(fetchError.response?.data?.message || "Unable to fetch your requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(submitError.response?.data?.message || "Unable to create request");
    } finally {
      setSubmitting(false);
    }
  };
