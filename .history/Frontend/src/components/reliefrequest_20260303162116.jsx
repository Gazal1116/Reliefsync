...
function ReliefRequests() {
  ...
  const handleSubmit = async (e) => {
    ...
    const newRequest = {
      ...
      // backend requires createdBy (ObjectId). Try to use stored user id, else use a fallback placeholder
      userId: (typeof window !== 'undefined' && localStorage.getItem('relief_user_id')) || '000000000000000000000000'
    };

    try {
      const res = await fetch("http://localhost:5000/api/relief/create", {
        method: "POST",
      ...
      if (res.ok && data.relief) {
        setRequests([...requests, data.relief]);
      } else {
        // fallback: add a local request so UI remains responsive
        const localReq = { ...newRequest, _id: `local-${Date.now()}`, status: 'Pending' };
        setRequests([...requests, localReq]);
      }
    } catch (err) {
      const localReq = { ...newRequest, _id: `local-${Date.now()}`, status: 'Pending' };
      setRequests([...requests, localReq]);
  ...
  const handleDelete = async (id) => {
    // If it's a local item, just remove locally
    ...
    try {
      await fetch(`http://localhost:5000/api/relief/delete/${id}`, {
        method: "DELETE"
      });
    } catch (err) {
  ...
  const toggleStatus = (id, currentStatus) => {
    ...
    // Backend does not expose an update endpoint for status; update locally for now
