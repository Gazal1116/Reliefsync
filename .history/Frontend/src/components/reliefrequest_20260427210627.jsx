...
import { ShieldCheck, MapPin, Plus, Activity, Loader2 } from "lucide-react";
...
const ReliefRequests = () => {
  ...
  const activeUserId = userId || userData.id || userData._id || getUserIdFromToken();
  ...
  const handleMarkCompletedFromList = async (requestId) => {
    try {
      setUpdatingRequestId(requestId);
      await axios.put(
        `http://localhost:5000/api/relief/update/${requestId}`,
        { status: "completed" },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const isOwnedByVolunteer = (item) => {
    ...
    return matchesId(createdBy) || matchesId(assignedTo) || matchesId(completedBy);
  };

  const volunteerRequests = requests.filter(isOwnedByVolunteer);

  // Show volunteer analytics only for the currently signed-in volunteer.
  const analyticsRequests = volunteerRequests;
                ...
                {requests.map((req) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <Link to={`/request/${req._id}`} className="font-semibold text-white hover:text-sky-300 transition-colors">
                          {req.title}
                        </Link>
                      ...
                      <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-semibold ${String(req.status).toLowerCase() === "completed" ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>
                        {String(req.status).toLowerCase() === "completed" ? "Completed" : "Pending"}
                      </span>
                    </div>
                    ...
                    {isVolunteer && (
                      <button
                        type="button"
                        onClick={() => handleMarkCompletedFromList(req._id)}
                        disabled={updatingRequestId === req._id || String(req.status).toLowerCase() === "completed"}
                        className="mt-3 inline-flex items-center justify-center gap-2 rounded-lg border border-emerald-400/30 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-200 transition-all hover:bg-emerald-500/20 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        {updatingRequestId === req._id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}
                        Mark as Completed
                      </button>
                    )}
                  </div>
          ...
          <div className="space-y-6">
            ...
            {isVolunteer && (
              <div className="bg-[#0a0a14] border border-white/10 p-6 rounded-2xl">
                ...
                {!requestsLoading && volunteerTotal === 0 && (
                  <p className="mb-3 text-xs text-sky-300">
                    Analytics is shown only for the currently signed-in volunteer.
                  </p>
                )}
